...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get("/categories", { params: { page: 1, limit: 1 } }),
      api.get("/brands", { params: { page: 1, limit: 1 } }),
      api.get("/users", { params: { page: 1, limit: 1 } }),
    ]).then(([products, categories, brands, users]) => {
      setStats({
        products: products.data.data.meta.total,
        categories:
          categories.data.data.length ?? categories.data.data.meta?.total,
        brands: brands.data.data.meta?.total ?? brands.data.data.length,
        users: users.data.data.meta?.total ?? users.data.data.length,
      });
    });
  }, []);
